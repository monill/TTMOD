...
if ($row["leechers"] >= 1 && $row["seeders"] >= 1 && $row["external"] != 'yes') {
    $speedQ = SQL_Query_exec("SELECT (SUM(p.downloaded)) / (UNIX_TIMESTAMP(NOW()) -  UNIX_TIMESTAMP(added)) AS totalspeed FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 'no' AND p.torrent = '$id' GROUP BY t.id ORDER BY added ASC LIMIT 15");
...
if ($row["banned"] == "yes") {
    ...
    if ($row["external"] == 'yes') {
        print ("<td width='160' align='right' valign='top'><table><tr>");
        if ($scrape == '1') {
            print("<td class='table_col1' width='150' valign='middle'>
            <div style='margin-top:5px; margin-bottom:10px; font:bold 16px Times' align='center'><font color='Green'>Update completed</font></div>");

            //===| UDP Scraper
            $resu = SQL_Query_exec("SELECT id, info_hash FROM torrents WHERE external = 'yes' AND id = $id");
            while ($rowu = mysqli_fetch_array($resu)) {
                $TorrentInfo = ParseTorrent("$site_config[torrent_dir]/$rowu[id].torrent");
                $ann = $TorrentInfo[0];
                $annlist = array();
                ...
                foreach ($annlist as $ann) {
                    ...
                    if ($stats['seeds'] != -1) {
                        ...
                        SQL_Query_exec("UPDATE `announce` SET `online` = 'yes', `seeders` = '" . $stats[seeds] . "', `leechers` = '" . $stats[peers] . "', `times_completed` = '" . $stats[downloaded] . "' WHERE `url` = " . sqlesc($ann) . " AND `torrent` = $rowu[id]");
                    } else {
                        SQL_Query_exec("UPDATE `announce` SET `online` = 'no' WHERE `url` = " . sqlesc($ann) . " AND `torrent` = $rowu[id]");
                    }
                }

                if ($seeders !== null) {
                    SQL_Query_exec("UPDATE torrents SET leechers = '" . $leechers . "', seeders = '" . $seeders . "', times_completed = '" . $downloaded . "', last_action = '" . get_date_time() . "', visible = 'yes' WHERE id = '" . $rowu['id'] . "'");
                } else {
                    SQL_Query_exec("UPDATE torrents SET last_action = '" . get_date_time() . "' WHERE id='" . $rowu['id'] . "'");
                }
            }
            //===| End UDP Scraper

            if ($seeders !== null) {
                print ("<div style='margin-bottom:5px; margin-left:20px'><b>" . T_("LIVE_STATS") . "</b>: </div>");
                print ("<div style='margin-left:20px'>" . T_("SEEDERS") . ": <font color='limegreen'>" . $seeders . "</font></div>");
                print ("<div style='margin-left:20px'>" . T_("LEECHERS") . ": <font color='#FF2000'>" . $leechers . "</font></div>");
                print ("<div style='margin-left:20px'>" . T_("COMPLETED") . ": <font color='#0080FF'>" . $downloaded . "</font></div>");
            } else {
                print ("<div style='margin-top:10px; margin-bottom:5px' align='center'><b>" . T_("LIVE_STATS") . "</b>: </div>");
                print ("<div style='font:italic 14px Times' align='center'><font color='purple'>No trackers :(</font></div>");
            }
            print ("<div style='margin-top:10px; margin-bottom:10px' align='center'>
                <a href='../torrents-details.php?id=" . $id . "'><input type='submit' name='submit' class='btn btn-block btn-success' value=" . T_("SUBMIT") . "></a>
            </div>");
            print ("</td>");
        } else {
            print ("<td class='table_col1' width='150' valign='middle' align='center'>
                <div style='margin-top:5px; margin-bottom:10px; font:bold 16px Times' align='center'><font color='black'>Update trackers</font></div>
                    <form action='../torrents-details.php?id=$id&amp;scrape=1' method='post'>
                        <div style='margin-top:5px; margin-bottom:10px' align='center'><input type='submit' class='btn btn-block btn-danger' name='submit' value=" . T_("UPDATE") . " /></div>
                    </form>
                </td>");
        }
        print ("</tr></table></td>");
